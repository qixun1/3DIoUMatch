...
def inside_bbox(point, corner_points):
    """
    Check if point is inside 3D bbox
    """
    i = (corner_points[0] - corner_points[1]).cpu().numpy()
    j = (corner_points[0] - corner_points[3]).cpu().numpy()
    k = (corner_points[0] - corner_points[4]).cpu().numpy()
    v = (corner_points[0] - point).cpu().numpy()
    first = np.logical_and(0 <= np.dot(v, i), np.dot(v, i) <= np.dot(i, i))
    second = np.logical_and(0 <= np.dot(v, j), np.dot(v, j) <= np.dot(j, j))
    temp = np.logical_and(first, second)
    third = np.logical_and(0 <= np.dot(v, k), np.dot(v, k) <= np.dot(k, k))

    return np.logical_and(temp, third)
...
def boxes3d_iou_batch(batch_corners1, batch_corners2):
    ...
    for i in range(m):
        rect2 = [(batch_corners2[i, k, 0], batch_corners2[i, k, 2]) for k in
                 range(3, -1, -1)]  # [((0,0),(0,2)),..., ((3,0),(3,2))]
        vol2 = vol_batch2[i]
