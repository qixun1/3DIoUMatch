...
def start_render(vtkActors, key_to_actors_to_hide={}, window_wh=(300,300), background_col=(0,0,0), background_col2=None,
...
def visualize_votes(example):
    ...
    vtk_gt_boxes = []
    vtk_pc = VtkPointCloud(example['point_clouds'].squeeze(0)[sa1_indices].detach().cpu().numpy(),
                               color=Color.Gray, point_size=5)
    vtk_actors.append(vtk_pc.vtk_actor)

    # Visualize sa1
    vtk_sa1_pred = VtkPointCloud(example['sa1_xyz'].squeeze(0)[sa2_indices].detach().cpu().numpy(), point_size=15,
                                     color=Color.Red)
    vtk_sa1.append(vtk_sa1_pred.vtk_actor)

    # Visualize sa2
    vtk_sa2_pred = VtkPointCloud(example['sa2_xyz'].squeeze(0)[bbox_indices].detach().cpu().numpy(), point_size=10,
                                     color=Color.Purple)
    vtk_sa2.append(vtk_sa2_pred.vtk_actor)

    # Visualize aggregated votes
    vtk_agg_votes = VtkPointCloud(example['aggregated_vote_xyz'].squeeze(0).detach().cpu().numpy(),
                                      point_size=5, color=Color.Green)
    vtk_aggregated_votes.append(vtk_agg_votes.vtk_actor)

    # Visualize GT 3D boxes
    for box3d in gt_box3d_list:
        vtk_box3D = vtk_box_3D(box3d, line_width=2, color=Color.Green)
        vtk_gt_boxes.append(vtk_box3D)

    key_to_actors_to_hide = {'g': vtk_aggregated_votes, 'p': vtk_sa1, 'k': vtk_sa2, 'b': vtk_gt_boxes}
    start_render(vtk_actors, key_to_actors_to_hide=key_to_actors_to_hide, background_col=Color.White)
...
def visualise_indices(bboxes, pc, indices):
    ...
    vtk_actors = []
    vtk_gt_boxes = []
    vtk_pc = VtkPointCloud(pc[indices],
                               color=Color.Gray, point_size=5)
    vtk_actors.append(vtk_pc.vtk_actor)
